...
Board::Board()
: board(3, vector<int>(3, 0)) {
    width = 150 * 3;
    height = 150 * 3;
    empty = 0;
    aiMarker.loadImage("x.png");
    humanMarker.loadImage("o.png");
}

Board::Board(unsigned int w, unsigned int h, int e)
            : board(3, vector<int>(3, e)) {
    width = w;
    height = h;
    empty = e;
}

void Board::DrawBoard(int top, int left, int square) {
    for (int row = 0; row < board.size(); row++) {
        for (int col = 0; col < board[row].size(); col++) {
            int box = board[row][col];
            int x = left + (square * col);
            int y = top + (square * row);
            if (box == 1) {
                //humanMarker.draw(x,y);
                printf("drew an human marker %d\n", (row *3) + col);
            } else if (box == 2) {
                //aiMarker.draw(x, y);
                printf("drew an ai marker %d\n", (row *3) + col);
            }
        }
    }
}
 ...
 */
ofVec2f Board::CoordToPos(unsigned int x,
...
void Board::Reset(){
    for(int i=0; i<board.size(); i++){
        int amount = board[i].size();
        board[i].erase(board[i].begin(), board[i].end());
        board[i].assign(amount, empty);
    }
}
...
vector<ofVec2f> Board::GetPossibleMoves(){
    vector<ofVec2f> x;
    for(int i=0; i<3; i++){
        for(int j=0; j<3; j++){
            if(board[i][j] == empty){
